...
try:
    ...
    for k,values in sentenceAnnotDict.iteritems():
        if isinstance(sentenceAnnotDict[k][0][1][], basestring):
            print 'hello'

        #print list(values[0])
        count = count + list(sentenceAnnotDict[1][1][1])
        #print count
    ...
    """
    #print sorted(percentValue)

    
    #print sorted(percentValueDict.items())

    writer = csv.writer(open('binary.csv', 'wb'))
    for key, value in sorted(sentenceAnnotDict.items()):
